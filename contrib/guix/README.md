# Bootstrappable Metrix Core Builds

This directory contains the files necessary to perform bootstrappable Metrix
Core builds.
...
To build Metrix Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Metrix Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/metrix-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/metrix-core/guix.sigs).
