Gitian building
================
*Setup instructions for a Gitian build of Metrix Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Metrix
...
- [Building Metrix Core](#building-metrix-core)
...
```bash
sudo cp /home/debian/metrix/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the metrix repository under 'Fetch and create inputs' to install sources which require
...
To build Metrix Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the metrix repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/metrix/.git/
...
For example:
```bash
URL=https://github.com/laanwj/metrix.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit metrix=${COMMIT} --url metrix=${URL} ../metrix/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit metrix=${COMMIT} --url metrix=${URL} ../metrix/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit metrix=${COMMIT} --url metrix=${URL} ../metrix/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-metrix:
./bin/gbuild --commit metrix=${COMMIT},cpp-eth-metrix=${ETHCOMMIT} --url metrix=${URL},cpp-eth-metrix=${ETHURL} ../metrix/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the metrix git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../metrix/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/TheLindaProjectInc/metrix-detached-sigs.git

BTCPATH=/some/root/path/metrix
SIGPATH=/some/root/path/metrix-detached-sigs

./bin/gbuild --url metrix=${BTCPATH},signature=${SIGPATH} ../metrix/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/metrix-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/metrix-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/metrix-osx-build.assert
```
...
[TheLindaProjectInc/gitian.sigs](https://github.com/TheLindaProjectInc/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@metrix.org) and he will commit them.
