...
BlockAssembler::Options::Options() {
    blockMinFeeRate = CFeeRate(DefaultBlockMinTxFee());
...
static BlockAssembler::Options DefaultOptions()
{
    ...
    if (gArgs.IsArgSet("-blockmintxfee") && ParseMoney(gArgs.GetArg("-blockmintxfee", ""), n)) {
        ...
        options.blockMinFeeRate = CFeeRate(DefaultBlockMinTxFee());
    }
    return options;
}
...
void BlockAssembler::RebuildRefundTransaction(){
    ...
    contrTx.vout[refundtx].nValue = nFees + GetBlockSubsidy(nHeight, chainparams.GetConsensus(), pblock->vtx[refundtx]);
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    if(fProofOfStake)
    {
        ...
        //////////////////////////////////////////////////////// metrix 
        AddCoinstakeContracts(&coinstakeTx);
        ////////////////////////////////////////////////////////
    ...
    if (!fProofOfStake)
    {
        // metrix we GenerateCoinbaseCommitment for POS blocks after we've executed
        // the coinstake contracts as they will effect the resulting witness
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    while (true)
    {
        ...
        if(setCoins.size() > 0)
        {
            ...
            std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params(), pwallet).CreateEmptyBlock(CScript(), true, true, &nTotalFees));
            if (!pblocktemplate.get())
                return;
            ...
            for(uint32_t i=beginningTime;i<beginningTime + nMaxStakeLookahead;i+=stakeTimestampMask+1) {
                ...
                if (SignBlock(pblock, *pwallet, nTotalFees, i, setCoins)) {
                    ...
                    std::unique_ptr<CBlockTemplate> pblocktemplatefilled(
                            BlockAssembler(Params(), pwallet).CreateNewBlock(pblock->vtx[1]->vout[1].scriptPubKey, true, true, &nTotalFees,
