...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-checklevel=<n>", strprintf(_("How thorough the block verification of -checkblocks is (0-4, default: %u)"), 1));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "metrix.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 33820));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BTC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));    strUsage += HelpMessageOpt("-keypool=<n>", strprintf(_("Set key pool size to <n> (default: %u)"), 100));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee per KB to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-txconfirmtarget=<n>", strprintf(_("If paytxfee is not set, include enough fee so transactions are confirmed on average within n blocks (default: %u)"),1));
    ...
    if (GetBoolArg("-help-debug", false)) {
        ...
        strUsage += HelpMessageOpt("-confchange" , strprintf(_("Require a confirmations for change (default: %u)"),0));
        strUsage += HelpMessageOpt("-minimizecoinage", strprintf(_("Minimize weight consumption (experimental) (default: %u)"),0));
        strUsage += HelpMessageOpt("-regtest", _("Enter regression test mode, which uses a special chain in which blocks can be "));
        strUsage += HelpMessageOpt("solved instantly. This is intended for regression testing tools and app development."));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, coinage, coinstake, creation, stakemodifier"; // Don't translate these and qt below
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees per KB smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-blockmaxsize=<n>", strprintf(_("Set maximum block size in bytes (default: %u)"),250000));
    strUsage += HelpMessageOpt("-blockprioritysize=<n>", strprintf(_("Set maximum size of high-priority/low-fee transactions in bytes (default: %u)"),27000));
    sstrUsage += HelpMessageOpt("-mininput=<amt>", strprintf(_("When creating transactions, ignore inputs with value less than this (default: %d)"),0.01));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 33821));
    strUsage += HelpMessageOpt("-rpcallowip=<ip>", _("Allow JSON-RPC connections from specified source. Valid for <ip> are a single IP (e.g. 1.2.3.4), a network/netmask (e.g. 1.2.3.4/255.255.255.0) or a network/CIDR (e.g. 1.2.3.4/24). This option can be specified multiple times"))    strUsage += "  -rpcconnect=<ip>       " + strprintf(_("Send commands to node running on <ip> (default: %s)"),"127.0.0.1") + "\n";
    ...
    strUsage +=  HelpMessageGroup( _("Block sync/database options:"));
    strUsage += HelpMessageOpt("-dbcache=<n>", strprintf(_("Set database cache size in megabytes (%d to %d, default: %d)"), nMinDbCache, nMaxDbCache, nDefaultDbCache));
    strUsage += HelpMessageOpt("-maxorphanblocks=<n>", strprintf(_("Keep at most <n> unconnectable blocks in memory (default: %u)"), DEFAULT_MAX_ORPHAN_BLOCKS));
    strUsage += HelpMessageOpt("-maxorphantx=<n>", strprintf(_("Keep at most <n> unconnectable transactions in memory (default: %u)"), DEFAULT_MAX_ORPHAN_TRANSACTIONS));
    strUsage += HelpMessageOpt("-stopafterblockimport", strprintf(_("Stop running after importing blocks from disk (default: %u)"));
    strUsage += HelpMessageOpt("-synctimeout=<n>", strprintf(_("Specify block download timeout in seconds (default: %u)"),60));
    strUsage += HelpMessageOpt("-synctime", strprintf(_("Sync time with other nodes. Disable if time on your system is precise e.g. syncing with NTP (default: %u)"),0));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Masternode and Darksend related functionality (0-1, default: %u)"),0));

    strUsage += HelpMessageOpt( _("Masternode options:"));
    strUsage += HelpMessageOpt("-masternode=<n>", strprintf(_("Enable the client to act as a masternode (0-1, default: %u)"),0));
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify masternode configuration file (default: %s)"),"masternode.conf"));
    strUsage += HelpMessageOpt("-masternodeprivkey=<n>", _("Set the masternode private key") + "\n";
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", _("Set external address:port to get to this masternode (example: address:port)"));
    strUsage += HelpMessageOpt("-masternodeminprotocol=<n>", strprintf(_("Ignore masternodes less than version (example: 70007; default : %u)"),0));

    strUsage += HelpMessageOpt( _("Darksend options:"));
    ...
    strUsage += HelpMessageOpt("  -anonymizeMetrixamount=<n>", strprintf(_("Keep N Metrix anonymized (default: %u)"),0));
    ...
    strUsage += HelpMessageOpt( _("InstantX options:"));
    ...
    strUsage += HelpMessageOpt("-instantxdepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"),1));
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-paytxfee")) {
        ...
        payTxFee = CFeeRate(nFeePerK, 100000);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Metrix Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Metrix Core to complete") << "\n";
   ...
   if (chainActive.Tip() == NULL) {
    ...
    if (nAnonymizeMetrixAmount > 999999)
        nAnonymizeMetrixAmount = 999999;
    if (nAnonymizeMetrixAmount < 2)
        nAnonymizeMetrixAmount = 2;
