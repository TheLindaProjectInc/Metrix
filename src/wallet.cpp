...
void CWalletTx::GetAmounts(list<COutputEntry>& listReceived, list<COutputEntry>& listSent, int64_t& nFee, string& strSentAccount, const isminefilter& filter) const
{
    LOCK(pwallet->cs_wallet);
    nFee = 0;
    listReceived.clear();
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsTrusted())
            {
                ...
                for (unsigned int i = 0; i < pcoin->vout.size(); i++)
                {
                    ...
                    if(IsSpent(wtxid, i) || !IsMine(pcoin->vout[i]) || !IsDenominated(vin)) continue;
...
bool CWallet::SelectCoinsMinConf(int64_t nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet) const
{
    ...
    BOOST_FOREACH(const COutput &output, vCoins)    
    {
        ...
        if (output.nDepth < (pcoin->IsFromMe(ISMINE_SPENDABLE | ISMINE_WATCH_ONLY) ? nConfMine : nConfTheirs))
            continue;
...
std::map<CTxDestination, int64_t> CWallet::GetAddressBalances()
{
...

    {
        ...
        BOOST_FOREACH(PAIRTYPE(uint256, CWalletTx) walletEntry, mapWallet)
        {
            ...
            int nDepth = pcoin->GetDepthInMainChain();
            if (nDepth < (pcoin->IsFromMe(ISMINE_SPENDABLE | ISMINE_WATCH_ONLY) ? 0 : 1))
                continue;
