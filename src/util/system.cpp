...
const char * const BITCOIN_CONF_FILENAME = "metrix.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MetrixCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MetrixCoin
    // Mac: ~/Library/Application Support/MetrixCoin
    // Unix: ~/.metrixcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MetrixCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MetrixCoin";
#else
    // Unix
    return pathRet / ".metrixcoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Metrix Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Metrix Core Developers";
    }
