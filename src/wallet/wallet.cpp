...
bool CWallet::SignTransaction(CMutableTransaction& tx)
{
    ...
    for (auto& input : tx.vin) {
        ...
        SignatureData sigdata;
        const SigningProvider* provider = GetSigningProvider();
        if (!provider || !ProduceSignature(*provider, MutableTransactionSignatureCreator(&tx, nIn, amount, SIGHASH_ALL), scriptPubKey, sigdata)) {
            return false;
        }
...
bool CWallet::CreateCoinStake(interfaces::Chain::Lock& locked_chain, const FillableSigningProvider& keystore, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins)
{
    ...
    CAmount nBudgetSubsidyFromCoinstake = 0;
    // Calculate reward
    {
    ...
    if (GetDGPVout(txNew.vout, qtumDGP.getBudgetDGP().asBytes(), ParseHex("104ad86f"), bgtVout, n))
    {
