...
bool BerkeleyEnvironment::Open(bool retry)
{
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance of metrix may be using it.\n", strPath);
        return false;
    }
...
bool BerkeleyBatch::VerifyDatabaseFile(const fs::path& file_path, std::string& errorStr)
{
    ...
    if (fs::exists(walletDir / walletFile))
    {
        if (!env->Verify(walletFile)) {
            errorStr = strprintf(_("%s corrupt. Try using the wallet tool metrix-wallet to salvage or restoring a backup.").translated, walletFile);
            return false;
        }
    }
