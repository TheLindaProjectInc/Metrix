    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getnewaddress",
                ...
                RPCResult{
            "\"address\"    (string) The new metrix address\n"
                },
                RPCExamples{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The metrix address to be associated with a label."},
...
static UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"sendtoaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The metrix address to send to."},
            ...
            "                             The recipient will receive less MRX than you enter in the amount field."},
            ...
            "                             dirty if they have previously been used in a transaction."},
                    {"senderaddress", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "The metrix address that will be used to send money from."},
    ...
    senderAddress = DecodeDestination(request.params[9].get_str());
        if (!IsValidDestination(senderAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Metrix address to send from");
        else
            fHasSender=true;
    }
...
static UniValue createcontract(const JSONRPCRequest& request){
    ...
    CAmount nGasPrice = (minGasPrice>DEFAULT_GAS_PRICE)?minGasPrice:DEFAULT_GAS_PRICE;
                ...
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"gasPrice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED, "gasPrice MRX price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                    {"senderaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::OMITTED, "The metrix address that will be used to create the contract."},
    ...
    CAmount nGasPrice = (minGasPrice>DEFAULT_GAS_PRICE)?minGasPrice:DEFAULT_GAS_PRICE;
                ...
                RPCHelpMan{"sendtocontract",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                    {
                        ...
                        {"gasPrice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED, "gasPrice Metrix price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                        {"senderaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::OMITTED, "The metrix address that will be used as sender."},
                        {"broadcast", RPCArg::Type::BOOL, RPCArg::Optional::OMITTED, "Whether to broadcast the transaction or not."},
                        {"changeToSender", RPCArg::Type::BOOL, RPCArg::Optional::OMITTED, "Return the change to the sender."},
                    },
                    RPCResult{
        ...
        return NullUniValue;
            ...
            RPCHelpMan{"signmessage",
                "\nSign a message with the private key of an address" +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The metrix address to use for the private key."},
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The metrix address for transactions."},
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{"sendmany",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, RPCArg::Optional::NO, "A json object with addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The metrix address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
            ...
            "                           Those recipients will receive less MRX than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Metrix address: ") + name_);
        }
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            HelpRequiringPassphrase(pwallet) + "\n",
            {
                ...
                {"amounts", RPCArg::Type::OBJ, RPCArg::Optional::NO, "A json object with addresses and amounts",
                    {
                        {"address", RPCArg::Type::STR_HEX, RPCArg::Optional::OMITTED, "The metrix address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                    },
                },
        ...
        return NullUniValue;
            ...
            RPCHelpMan{"addmultisigaddress",
                ...
                "If 'label' is specified, assign address to that label.\n",
                {
                    ...
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of metrix addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "metrix address or hex-encoded public key"},
                        },
                        },
...
static UniValue gettransaction(const JSONRPCRequest& request_)
{
    ...
    if (!shouldWaitConf) {
        ...
        if(!request.req)
            throw JSONRPCError(RPC_INTERNAL_ERROR, "No HTTP connection. Waitconf is available from metrix-cli, not metrix-qt");
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"walletpassphrase",
                ...
                "This is needed prior to performing transactions related to private keys such as sending MRX and staking\n"
            "\nNote:\n"
...
static UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending metrix\n"
...
static UniValue loadwallet(const JSONRPCRequest& request)
{
            RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting metrixd will be"
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "A json array of metrix addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "metrix address"},
                        },
                    },
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Metrix address: ") + input.get_str());
            }
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid metrix address");
            }
    ...
    RPCHelpMan{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* default */ "pool address", "The metrix address to receive the change"},
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getaddressinfo",
                "\nReturn information about the given metrix address. Some information requires the address\n"
                "to be in the wallet.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The metrix address to get the information of."},
                },
                RPCResult{
            "{\n"
                ...
                "Implements the Creator and Updater roles.\n",
                {
                    ...
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "a json array with outputs (key-value pairs), where none of the keys are duplicated.\n"
                            ...
                            "                             accepted as second parameter.",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the metrix address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                                },
                                },
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                        {
                            {"changeAddress", RPCArg::Type::STR_HEX, /* default */ "pool address", "The metrix address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM, /* default */ "random", "The index of the change output"},
                            {"change_type", RPCArg::Type::STR, /* default */ "set by -changetype", "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
                            {"includeWatching", RPCArg::Type::BOOL, /* default */ "true for watch-only wallets, otherwise false", "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
...
////////////////////////////////////////////////// // metrix
UniValue addressbookadd(const JSONRPCRequest& request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CWallet* const pwallet = wallet.get();
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"addressbookadd",
                ...
                RPCResults{},
                RPCExamples{
                HelpExampleCli("addressbookadd", "1PSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc Satoshi")
                },
            }.Check(request);
    ...
    // Check for duplicate addresses
    {
        LOCK(pwallet->cs_wallet);
        if (pwallet->mapAddressBook.count(dest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Address already in address book");
    }
...
UniValue addressbookremove(const JSONRPCRequest& request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CWallet* const pwallet = wallet.get();
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"addressbookremove",
                "\nRemove sending Metrix address from the address book." +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address."}
                },
                RPCResults{},
                RPCExamples{
                HelpExampleCli("addressbookremove", "1PSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc")
                },
            }.Check(request);
    ...
    {
        LOCK(pwallet->cs_wallet);
        if (pwallet->mapAddressBook.count(dest) && !IsMine(*pwallet, dest))
            pwallet->DelAddressBook(dest);
    }

    return NullUniValue;
}
UniValue listaddressbook(const JSONRPCRequest& request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CWallet* const pwallet = wallet.get();
    ...
    UniValue jsonBook(UniValue::VARR);
    for (const auto& item : pwallet->mapAddressBook)
    {
        if (!IsMine(*pwallet, item.first))
        {
            UniValue obj(UniValue::VOBJ);
            std::string address = EncodeDestination(item.first);
            obj.pushKV("address", address);
            obj.pushKV("label", item.second.name);
            jsonBook.push_back(obj);
        }
    }
    return jsonBook;
}
...
{ //  category              name                                actor (function)                argNames
    ...
    { "wallet",             "addressbookadd",                   &addressbookadd,                {"address","label"} },
    { "wallet",             "addressbookremove",                &addressbookremove,             {"address"} },
