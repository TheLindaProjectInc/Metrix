...
std::string HelpMessageCli()
{
    ...
    strUsage += "  -regtest               " + _("Enter regression test mode, which uses a special chain in which blocks can be ") +
                                                _("solved instantly. This is intended for regression testing tools and app development") + ".\n";
    strUsage += "  -rpcconnect=<ip>       " + strprintf(_("Send commands to node running on <ip> (default: %s)"), "127.0.0.1") + "\n";
    strUsage += "  -rpcport=<port>        " + strprintf(_("Connect to JSON-RPC on <port> (default: %u)"),33820) + "\n";
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        ...
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  bitcoin-cli [options] <command> [params]  " + _("Send command to Metrix Core") + "\n" +
